...
"""
Using test dump data along with spine labels, calculate a list where each entry specifies how many spines a particular bounding box encapsulates (expect values between 0 and 5) 
"""	
def generatePredHistData():
	...
	for imgName in imgToBox:
		...
		spineTips = []
		#print(os.path.join("labels+", expNum, imgName+".csv"))
...
def generateGTHistData(images):
	...
	for imgName in images:
		dataPath = os.path.join(currDir,"mmdetection","training","labels2D+_xyxy",imgName+".csv")
...
def overlapBoxes():
	imgToBox = {}
	pathToPreds = os.path.join(currDir, 'output', 'fRCNN_X101', "predBoxes_josh_fRCNN_X101_test_cropped.txt")
	with open(pathToPreds) as f:
		imgToBox = json.load(f)["data"]
	#print(list(imgToBox.keys())[0])
	#filenameDrop = "_".join(filename.split("_")[0:2])
	...
	dataPath = os.path.join(currDir,"spineData","imagesJosh2D_croppedResize","labels(xywh)",filename+".csv")
	GTBoxes = list(csv.reader(open(dataPath)))[1:]
	#print(predBoxes[0], GTBoxes[0])
	imagePath = os.path.join(currDir,"spineData/imagesJosh2D_croppedResize/test",filename+".tif") 
	# old, little bit of a bait
	...
	for box in predBoxes:
		...
		rect = patches.Rectangle(origin,width,height,linewidth=lw,facecolor='None',edgecolor=(1, 0, 1, 1), angle=0)
		ax.add_patch(rect)
	...
	for boxPre in GTBoxes:
			...
			rect = patches.Rectangle(origin,width,height,linewidth=lw,edgecolor='None',facecolor=(1, 1, 0, 0.4), angle=0)
			ax.add_patch(rect)
	...
	outputDir = os.path.join(os.path.dirname(currDir), 'results/detectron/fRCNN_X101/samples_test_josh_cropped')
...
def generateHistogram(spinePerBoxPred, spinePerBoxGT):
	...
	#plt.hist(spinePerBoxPred, bins=100, , range=(0,6))
